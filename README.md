...
opcionalmente indicando um tema específico. `.claude/settings.json` já
libera as permissões necessárias pro fluxo (WebFetch/WebSearch dos
domínios usados, download de imagem livre, tools de browser), então o
fluxo roda sem prompts de aprovação.
...
`claude.ai/code/routines` sobre este mesmo repositório
...
- **Não consegue**: abrir o Chrome e preencher o LinkedIn — isso depende
  ...
  não existe do lado da nuvem.

Fluxo típico: a rotina prepara o rascunho automaticamente 3x/semana → você
dá um `git pull` localmente → pede pro Claude Code local preencher esse
rascunho no LinkedIn e revisa antes de publicar.
...
│       └── images/            # imagens baixadas pela rotina na nuvem
└── sources/
